...
import com.kan.hro.service.inf.biz.sb.SBDetailTempService;
import com.kan.hro.service.inf.biz.sb.SBHeaderTempService;

public class SBDetailTempAction extends BaseAction
{
   @Override
   public ActionForward list_object( final ActionMapping mapping, final ActionForm form, final HttpServletRequest request, final HttpServletResponse response ) throws KANException
   {
      try
      {
         ...
         final SBDetailTempService sbDetailTempService = ( SBDetailTempService ) getService( "sbDetailTempService" );

         // 获得批次主键ID
         final String headerId = KANUtil.decodeStringFromAjax( request.getParameter( "headerId" ) );
         final SBHeaderTempVO sbHeaderTempVO = sbHeaderTempService.getSBHeaderTempVOByHeaderId( headerId );
         ...
         sbDetailTempService.getSBDetailTempVOsByCondition( sbDetailTempHolder, true );
         ...
         throws KANException
   {
      try
      {
         ...
         final SBDetailTempVO sbDetailTempVO = sbDetailTempService.getSBDetailTempVOByDetailId( detailId );
         ...
         throws KANException
   {
      try
      {
         ...
         sbDetailTempVO.setAmountCompany( KANUtil.filterEmpty( tempSBSBDetailTempVO.getAmountCompany() ) == null ? "0" : tempSBSBDetailTempVO.getAmountCompany() );
         sbDetailTempVO.setAmountPersonal( KANUtil.filterEmpty( tempSBSBDetailTempVO.getAmountPersonal() ) == null ? "0" : tempSBSBDetailTempVO.getAmountPersonal() );

         sbDetailTempService.updateSBDetailTemp( sbDetailTempVO );
         // 发送更新成功标记
      ...
      try
      {
         ...
         if ( sbDetailTempVO.getSelectedIds() != null && !sbDetailTempVO.getSelectedIds().trim().isEmpty() )
         {
            ...
            for ( String selectedId : sbDetailTempVO.getSelectedIds().split( "," ) )
            {
               ...
               {
                  // 调用删除接口
                  sbDetailTempService.deleteSBDetailTemp( KANUtil.decodeStringFromAjax( selectedId ) );
               }
            }
