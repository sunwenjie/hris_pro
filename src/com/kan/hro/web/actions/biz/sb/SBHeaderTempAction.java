...
public class SBHeaderTempAction extends BaseAction
{
   @Override
   public ActionForward list_object( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws KANException
   {
      try
      {
         ...
         sbHeaderTempService.getSBHeaderTempVOsByCondition( pagedListHolder, true );
         // 刷新国际化
   ...
   public ActionForward to_sbHeaderTemp( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws KANException
   {
      ...
      // 初始化Service接口
      final CommonBatchService commonBatchService = ( CommonBatchService ) getService( "sbHeaderTempService" );
      final SBHeaderTempService sbHeaderTempService = ( SBHeaderTempService ) getService( "sbHeaderTempService" );
      ...
      try
      {
         ...
         if ( sbHeaderTempVO.getSelectedIds() != null && !sbHeaderTempVO.getSelectedIds().trim().isEmpty() )
         {
            ...
            for ( String selectedId : sbHeaderTempVO.getSelectedIds().split( "," ) )
            {
               ...
               {
                  // 初始化SBHeaderTempVO
                  final SBHeaderTempVO tempSBHeaderTempVO = sbHeaderTempService.getSBHeaderTempVOByHeaderId( KANUtil.decodeStringFromAjax( selectedId ) );
                  // 调用删除接口
                  tempSBHeaderTempVO.setModifyBy( getUserId( request, response ) );
                  tempSBHeaderTempVO.setModifyDate( new Date() );
                  tempSBHeaderTempVO.setVendorId( getVendorId( request, response ) );
                  tempSBHeaderTempVO.setCorpId( getCorpId( request, response ) );
                  rows = rows + sbHeaderTempService.updateSBHeaderTemp( tempSBHeaderTempVO );
               }
      ...
      try
      {
         ...
         if ( sbHeaderTempVO.getSelectedIds() != null && !sbHeaderTempVO.getSelectedIds().trim().isEmpty() )
         {
            ...
            for ( String selectedId : sbHeaderTempVO.getSelectedIds().split( "," ) )
            {
               ...
               {
                  // 调用删除接口
                  sbHeaderTempService.deleteSBHeaderTemp( KANUtil.decodeStringFromAjax( selectedId ) );
               }
            }
