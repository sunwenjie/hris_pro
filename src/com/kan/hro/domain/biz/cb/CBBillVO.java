 ...
 * 创建时间：2013-9-12  
 */
public class CBBillVO extends BaseVO
{
   ...
   // 派遣协议状态
   private List< MappingVO > additionalStatuses = new ArrayList< MappingVO >();
   ...
   public void reset( final ActionMapping mapping, final HttpServletRequest request )
   {
      ...
      this.additionalStatuses.addAll( KANUtil.getMappings( this.getLocale(), "business.cb.header.status" ) );
      try
      {
         if ( BaseAction.getRole( request, null ).equals( KANConstants.ROLE_CLIENT ) )
         {
            List< MappingVO > clientStatus = new ArrayList< MappingVO >();
            List< MappingVO > dbStatus = KANUtil.getMappings( this.getLocale(), "business.cb.header.status" );
            for ( MappingVO mappingVO : dbStatus )
            {
               if ( !( "1".equals( mappingVO.getMappingId() ) || "2".equals( mappingVO.getMappingId() ) ) )
               {
                  clientStatus.add( mappingVO );
               }
            }
            super.setStatuses( clientStatus );
         }
      }
   ...
   {
      return decodeField( this.additionalStatus, additionalStatuses );
   }
   ...
   public List< MappingVO > getAdditionalStatuses()
   {
      return additionalStatuses;
   }

   public void setAdditionalStatuses( List< MappingVO > additionalStatuses )
   {
   ...
   public String getDecodeDetailAmountPurchaseCosts()
   {
      double sum = 0;
      if ( detailList != null && detailList.size() > 0 )
      {
         ...
         {
            sum += Double.parseDouble( formatNumber( ( ( CBBillVO ) obj ).getAmountPurchaseCost() ) );
         }
      }
