...
<div id="content">
	<div class="box searchForm toggableForm" id="sbBatch-information">
		<div class="head"><label><bean:message bundle="sb" key="sb.title.preview" /></label></div>
	...
	<div class="box noHeader" id="search-results">
		<div class="inner">
			...
			<div class="top">
				<kan:auth right="new" action="<%=SBAction.ACCESS_ACTION_PREVIEW%>">
					<input type="button" class="" id="btnAdd" name="btnAdd" value="<bean:message bundle="public" key="button.create" />" onclick="link('sbAction.do?proc=to_estimationNew');" />
				</kan:auth>
				<kan:auth right="approve" action="<%=SBAction.ACCESS_ACTION_PREVIEW%>">
					<input type="button" class="function" name="btnApprove" id="btnApprove" value="<bean:message bundle="public" key="button.approve" />" />
				</kan:auth>
				<kan:auth right="back" action="<%=SBAction.ACCESS_ACTION_PREVIEW%>">
			...
			<!-- Include table jsp 包含tabel对应的jsp文件 -->  
				<jsp:include page="/contents/sb/table/listSBBatchTablePreView.jsp" flush="true"/> 
			</div>
			<!-- tableWrapper -->
			<div class="bottom">
				<p>
		...
		$('#btnApprove').click(function(){
			if($('#selectedIds').val() != null && $('#selectedIds').val() != ''){
				if(confirm('<bean:message bundle="public" key="popup.confirm.approve.selected.batch" />')){
					$('.list_form').attr('action', 'sbAction.do?proc=submit_estimation');
