...
<table class="table hover" id="resultTable">
	<thead>
		<tr>
			...
			<th style="width: 6%" class="header <%=sbAdjustmentHeaderHolder.getCurrentSortClass("adjustmentHeaderId")%>">
				<a onclick="submitForm('searchSBAdjustmentHeader_form', null, null, 'adjustmentHeaderId', '<%=sbAdjustmentHeaderHolder.getNextSortOrder("adjustmentHeaderId")%>', 'tableWrapper');"><bean:message bundle="sb" key="sb.adjustment.header.id" /></a>
			</th>
			<th style="width: 8%" class="header <%=sbAdjustmentHeaderHolder.getCurrentSortClass("employeeId")%>">
			...
			<th style="width: <%=request.getAttribute( "role" ).equals( "1" ) ? "6" : "6" %>" class="header <%=sbAdjustmentHeaderHolder.getCurrentSortClass("employeeNameZH")%>">
			...
			<th style="width: <%=request.getAttribute( "role" ).equals( "1" ) ? "6" : "6" %>%" class="header <%=sbAdjustmentHeaderHolder.getCurrentSortClass("employeeNameEN")%>">
			...
			<logic:equal name="role" value="1">
				<th style="width: 8%" class="header <%=sbAdjustmentHeaderHolder.getCurrentSortClass("clientId")%>">
			...
			<logic:equal name="role" value="1">
				<th style="width: 8%" class="header <%=sbAdjustmentHeaderHolder.getCurrentSortClass("orderId")%>">
			...
			<logic:equal name="role" value="2">
				<th style="width: 8%" class="header <%=sbAdjustmentHeaderHolder.getCurrentSortClass("orderDescription")%>">
			...
			<logic:equal name="role" value="1">
				<th style="width: 8%" class="header <%=sbAdjustmentHeaderHolder.getCurrentSortClass("contractId")%>">
			...
			<th style="width: 15%" class="header <%=sbAdjustmentHeaderHolder.getCurrentSortClass("sbSolutionId")%>">
			...
			<th style="width: 8%" class="header <%=sbAdjustmentHeaderHolder.getCurrentSortClass("status")%>">
		...
		<tbody id="mytab">
			<logic:iterate id="sbAdjustmentHeaderVO" name="sbAdjustmentHeaderHolder" property="source" indexId="number">
				<tr class='<%=number % 2 == 1 ? "odd" : "even"%>'>
					...
					<td class="left">
						<a onclick="link('sbAdjustmentDetailAction.do?proc=list_object_confirm&id=<bean:write name="sbAdjustmentHeaderVO" property="encodedId"/>');">
							<bean:write name="sbAdjustmentHeaderVO" property="adjustmentHeaderId" />
						</a>
					</td>
					<td class="left">
					...
					<td class="left"><bean:write name="sbAdjustmentHeaderVO" property="monthly" /></td>
					<td class="left"><bean:write name="sbAdjustmentHeaderVO" property="decodeSbSolutionId" /></td>
					...
					<td class="left">
						...
						<kan:auth right="submit" action="<%=SBAdjustmentHeaderAction.accessAction%>">
							<logic:equal name="sbAdjustmentHeaderVO" property="status" value="1" >
								&nbsp;&nbsp;<a onclick="submit_object('<bean:write name="sbAdjustmentHeaderVO" property="encodedId"/>')">提交</a>
							</logic:equal>
