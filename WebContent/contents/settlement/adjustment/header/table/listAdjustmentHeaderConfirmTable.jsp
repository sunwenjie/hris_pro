	...
	<thead>
		<tr>
			...
			<th style="width: 7%" class="header <%=adjustmentHeaderHolder.getCurrentSortClass("contractId")%>">
				<a onclick="submitForm('searchAdjustmentHeader_form', null, null, 'contractId', '<%=adjustmentHeaderHolder.getNextSortOrder("contractId")%>', 'tableWrapper');"><logic:equal name="role" value="1">派送信息</logic:equal><logic:equal name="role" value="2">劳动合同</logic:equal>ID</a>
			</th>
		...
		<tbody id="mytab">
			<logic:iterate id="adjustmentHeaderVO" name="adjustmentHeaderHolder" property="source" indexId="number">
				<tr class='<%=number % 2 == 1 ? "odd" : "even"%>'>
					...
					<td class="left">
						<a onclick="link('adjustmentDetailAction.do?proc=list_object_confirm&id=<bean:write name="adjustmentHeaderVO" property="encodedId"/>');">
