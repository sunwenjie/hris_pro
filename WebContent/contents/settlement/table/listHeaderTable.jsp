	...
	<thead>
		<tr>
			...
			<th style="width: 6%;" class="header <%=headerListHolder.getCurrentSortClass("orderHeaderId")%>">
				<a onclick="submitForm('searchHeader_form', null, null, 'orderHeaderId', '<%=headerListHolder.getNextSortOrder("orderHeaderId")%>', 'tableWrapper');">序号</a>
			</th>
			<th style="width: 6%;" class="header <%=headerListHolder.getCurrentSortClass("monthly")%>">
				<a onclick="submitForm('searchHeader_form', null, null, 'monthly', '<%=headerListHolder.getNextSortOrder("monthly")%>', 'tableWrapper');">结算月份</a>
			</th>
			<th style="width: 7%;" class="header <%=headerListHolder.getCurrentSortClass("clientId")%>">
				<a onclick="submitForm('searchHeader_form', null, null, 'clientId', '<%=headerListHolder.getNextSortOrder("clientId")%>', 'tableWrapper');">客户ID</a>
			</th>
			<th style="width: 25%;" class="header <%=headerListHolder.getCurrentSortClass("clientNameZH")%>">
				<a onclick="submitForm('searchHeader_form', null, null, 'clientNameZH', '<%=headerListHolder.getNextSortOrder("clientNameZH")%>', 'tableWrapper');">客户名称</a>
			</th>
			<th style="width: 7%;" class="header <%=headerListHolder.getCurrentSortClass("orderId")%>">
				<a onclick="submitForm('searchHeader_form', null, null, 'orderId', '<%=headerListHolder.getNextSortOrder("orderId")%>', 'tableWrapper');">订单ID</a>
			</th>
			<th style="width: 8%;" class="header <%=headerListHolder.getCurrentSortClass("billAmountCompany")%>">
				<a onclick="submitForm('searchHeader_form', null, null, 'billAmountCompany', '<%=headerListHolder.getNextSortOrder("billAmountCompany")%>', 'tableWrapper');">公司营收</a>
			</th>
			<th style="width: 8%;" class="header <%=headerListHolder.getCurrentSortClass("costAmountCompany")%>">
				<a onclick="submitForm('searchHeader_form', null, null, 'costAmountCompany', '<%=headerListHolder.getNextSortOrder("costAmountCompany")%>', 'tableWrapper');">公司成本</a>
			</th>
			<th style="width: 8%;" class="header <%=headerListHolder.getCurrentSortClass("billAmountPersonal")%>">
				<a onclick="submitForm('searchHeader_form', null, null, 'billAmountPersonal', '<%=headerListHolder.getNextSortOrder("billAmountPersonal")%>', 'tableWrapper');">个人收入</a>
			</th>
			<th style="width: 8%;" class="header <%=headerListHolder.getCurrentSortClass("costAmountPersonal")%>">
				<a onclick="submitForm('searchHeader_form', null, null, 'costAmountPersonal', '<%=headerListHolder.getNextSortOrder("costAmountPersonal")%>', 'tableWrapper');">个人支出</a>
			</th>
		...
		<tbody id="mytab">
			<logic:iterate id="headerVO" name="headerListHolder" property="source" indexId="number">
				<tr class='<%=number % 2 == 1 ? "odd" : "even"%>'>
					...
					<td class="left">雇员人次：<bean:write name="headerVO" property="countContractId" />；科目：<bean:write name="headerVO" property="countItemId" /></td>
				</tr>
			</logic:iterate>
		</tbody>
	</logic:notEqual>
		...
		<tfoot>
			<tr class="total">
				<td colspan="13" class="left">
					...
					<label>&nbsp;&nbsp;跳转至：<input type="text" id="forwardPage" class="forwardPage" style="width:23px;" value="<bean:write name="headerListHolder" property="realPage" />" onkeydown="if(event.keyCode == 13){forward();}" />页</label>&nbsp;
				</td>
			</tr>
		</tfoot>
	</logic:present>
