	...
	<thead>
		<tr>
			<th class="checkbox-col">
				...
				<input type="checkbox" id="kanList_" name="chkSelectAll" value="" style="display:none"/>
			</th>
		...
		<tbody id="mytab">
			<logic:iterate id="headerVO" name="headerListHolder" property="source" indexId="number">
				<tr class='<%=number % 2 == 1 ? "odd" : "even"%>'>
					<td>
							<input type="checkbox" id="kanList_chkSelectRecord_<bean:write name="headerVO" property="invoiceId"/>" checked="checked"  style="display:none"  name="chkSelectRow[]" value="<bean:write name="headerVO" property="encodedId"/>" />
					</td>
					<td class="left"><a onclick="link('systemInvoiceDetailAction.do?proc=list_object&pageFlag=Merge&invoiceId=<bean:write name="headerVO" property="encodedId"/>&batchId=<bean:write name="headerVO" property="encodedBatchId"/>');"><bean:write name="headerVO" property="invoiceId" /></a></td>
...
<script type="text/javascript">
	(function($) {
		...
		var arrChk=$("input[id^='kanList_chkSelectRecord_']:checked");
		
		var selectedIdArray=new Array();
		$(arrChk).each(function(i) {
			selectedIdArray[i]=this.value;
		});
		$(".listComInvoice_form #invoiceId").val(selectedIdArray);
	})(jQuery);	
</script>
