	...
	<thead>
		<tr>
			...
			<th style="width: 6%;" class="header <%=headerListHolder.getCurrentSortClass("invoiceId")%>">
				<a onclick="submitForm('listHeader_form', null, null, 'invoiceId', '<%=headerListHolder.getNextSortOrder("invoiceId")%>', 'tableWrapper');">序号</a>
			</th>
			<th style="width: 6%;" class="header <%=headerListHolder.getCurrentSortClass("monthly")%>">
				<a onclick="submitForm('listHeader_form', null, null, 'monthly', '<%=headerListHolder.getNextSortOrder("monthly")%>', 'tableWrapper');">月份</a>
			</th>
			<th style="width: 7%;" class="header <%=headerListHolder.getCurrentSortClass("clientId")%>">
				<a onclick="submitForm('listHeader_form', null, null, 'clientId', '<%=headerListHolder.getNextSortOrder("clientId")%>', 'tableWrapper');">客户ID</a>
			</th>
			<th style="width: 25%;" class="header <%=headerListHolder.getCurrentSortClass("clientNameZH")%>">
				<a onclick="submitForm('listHeader_form', null, null, 'clientNameZH', '<%=headerListHolder.getNextSortOrder("clientNameZH")%>', 'tableWrapper');">客户名称</a>
			</th>
			<th style="width: 17%;" class="header <%=headerListHolder.getCurrentSortClass("orderId")%>">
				<a onclick="submitForm('listHeader_form', null, null, 'orderId', '<%=headerListHolder.getNextSortOrder("orderId")%>', 'tableWrapper');">订单ID</a>
			</th>
			<th style="width: 8%;" class="header <%=headerListHolder.getCurrentSortClass("billAmountCompany")%>">
				<a onclick="submitForm('listHeader_form', null, null, 'billAmountCompany', '<%=headerListHolder.getNextSortOrder("billAmountCompany")%>', 'tableWrapper');">公司营收</a>
			</th>
			<th style="width: 8%;" class="header <%=headerListHolder.getCurrentSortClass("costAmountCompany")%>">
				<a onclick="submitForm('listHeader_form', null, null, 'costAmountCompany', '<%=headerListHolder.getNextSortOrder("costAmountCompany")%>', 'tableWrapper');">公司成本</a>
			</th>
			<th style="width: 8%;" class="header <%=headerListHolder.getCurrentSortClass("taxAmount")%>">
				<a onclick="submitForm('listHeader_form', null, null, 'taxAmount', '<%=headerListHolder.getNextSortOrder("taxAmount")%>', 'tableWrapper');">个税</a>
			</th>
				...
				<tr class='<%=number % 2 == 1 ? "odd" : "even"%>'>
					<td>
					 	...
					 	<logic:equal name="accountId" value="1">
							<logic:equal name="headerVO" property="extended" value="2">
						...
						<!-- Common Account -->
						 <logic:notEqual name="accountId" value="1">
							<logic:notEqual name="headerVO" property="accountId" value="1">
								<logic:equal name="headerVO" property="extended" value="2"> 
									<input type="checkbox" id="kanList_chkSelectRecord_<bean:write name="headerVO" property="invoiceId"/>" name="chkSelectRow[]" value="<bean:write name="headerVO" property="encodedId"/>" />
								 </logic:equal>
							</logic:notEqual>
					...
					<td class="left"><a onclick="link('systemInvoiceDetailAction.do?proc=list_object&pageFlag=<bean:write name="pageFlag" />&invoiceId=<bean:write name="headerVO" property="encodedId"/>&batchId=<bean:write name="headerVO" property="encodedBatchId"/>');"><bean:write name="headerVO" property="invoiceId" /></a></td>
	...
	<logic:present name="headerListHolder">
		<tfoot>
			<tr class="total">
				...
				<label>&nbsp;&nbsp;<a onclick="submitForm('listHeader_form', null, '<bean:write name="headerListHolder" property="firstPage" />', null, null, 'tableWrapper');"><bean:message bundle="public" key="page.first" /></a></label> 
				<label>&nbsp;<a onclick="submitForm('listHeader_form', null, '<bean:write name="headerListHolder" property="previousPage" />', null, null, 'tableWrapper');"><bean:message bundle="public" key="page.previous" /></a></label> 
				<label>&nbsp;<a onclick="submitForm('listHeader_form', null, '<bean:write name="headerListHolder" property="nextPage" />', null, null, 'tableWrapper');"><bean:message bundle="public" key="page.next" /></a></label> 
				<label>&nbsp;<a onclick="submitForm('listHeader_form', null, '<bean:write name="headerListHolder" property="lastPage" />', null, null, 'tableWrapper');"><bean:message bundle="public" key="page.last" /></a></label> 
