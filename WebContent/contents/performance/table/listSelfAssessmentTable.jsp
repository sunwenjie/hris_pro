...
<table class="table hover" id="resultTable">
	<thead>
		<tr>
			...
			<th class="header <%=selfAssessmentHolder.getCurrentSortClass("rating_final")%>">
				<a onclick="submitForm('listSelfAssessment_form', null, null, 'rating_final', '<%=selfAssessmentHolder.getNextSortOrder("rating_final")%>', 'tableWrapper');">
					<bean:message bundle="performance" key="emp.self.assessment.final.rating" />
				</a>
			</th>
		...
		<tbody>
			<logic:iterate id="selfAssessmentVO" name="selfAssessmentHolder" property="source" indexId="number">
				<tr class='<%= number % 2 == 1 ? "odd" : "even" %>'>
					...
					<td class="left">
						<a onclick="link('selfAssessmentAction.do?proc=to_objectModify&id=<bean:write name="selfAssessmentVO" property="encodedId" />')">
					...
					<td class="left"><bean:write name="selfAssessmentVO" property="rating_final"/></td>
					<td class="left"><bean:write name="selfAssessmentVO" property="decodeStatus"/>
					   <logic:equal value="true" name="isHR">
					   <a onclick="popupSelfAssessment(${selfAssessmentVO.assessmentId});" class="kanhandle"><img src="images/search.png" title="<bean:message bundle="public" key="button.search" />" /></a>
					   </logic:equal>
					</td>
				</tr>
			</logic:iterate>
...
<div id="popupWrapper">
    <jsp:include page="/popup/showSelfAssessmentProgess.jsp"></jsp:include>
</div>