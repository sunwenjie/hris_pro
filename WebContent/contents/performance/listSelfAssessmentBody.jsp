...
<div id="content">
	<div class="box searchForm toggableForm" id="bank-information">
		<div class="head">
			<label><bean:message bundle="performance" key="emp.self.assessment" /> </label>
		</div>
		<!-- inner -->
		...
		<div id="searchDiv" class="inner">
			...
			<html:form action="selfAssessmentAction.do?proc=list_object" styleClass="listSelfAssessment_form">
				...
				<fieldset>
					<ol class="auto">
						...
						<li>
							<label><bean:message bundle="performance" key="emp.self.assessment.isDirectSub" /></label> 
							<html:select property="isDirectSub" styleClass="search_isDirectSub">
	...
	<div class="box noHeader" id="search-results">
		<div class="inner">
			...
			<div class="top">
				<kan:auth right="new" action="<%=SelfAssessmentAction.ACCESS_ACTION%>">
					<input type="button" class="" id="btnAdd" name="btnAdd" value="<bean:message bundle="public" key="button.add" />" onclick="link('selfAssessmentAction.do?proc=to_objectNew');" />
				</kan:auth>
				<logic:equal value="true" name="isHR">
                    <input type="button" class="function" id="exportProgress" name="exportProgress" value="<bean:message bundle="public" key="button.export.progress" />" onclick="exportProgress();" />
                </logic:equal>
				<a id="filterRecords" name="filterRecords" class="inquiry"><bean:message bundle="public" key="set.filerts" /></a>
				<kan:auth right="export" action="<%=SelfAssessmentAction.ACCESS_ACTION%>">
					<a id="exportExcel" name="exportExcel" class="commonTools" title="<bean:message bundle="public" key="img.title.tips.export.excel" />" onclick="linkForm('listSelfAssessment_form', 'downloadObjects', null, 'fileType=excel');">
						<img src="images/appicons/excel_16.png" />
					</a>
				</kan:auth>
				
				<kan:auth right="import" action="<%=SelfAssessmentAction.ACCESS_ACTION%>">
					<a id="importExcel" name="importExcel" class="commonTools" onclick='popupExcelImport();'>
						<img style='float:right' src='images/import.png'  title='<bean:message bundle="public" key="img.title.tips.import.excel" />' />
					</a>
				</kan:auth>
			</div>
			<!-- top -->
			<div id="tableWrapper">
				...
				<jsp:include page="/contents/performance/table/listSelfAssessmentTable.jsp" flush="true"/> 
			</div>
			<!-- tableWrapper -->
			<div class="bottom">
				<p>
...
<div id="popupWrapper">
	...
	<jsp:include page="/popup/importSelfAssessment.jsp" flush = "true">
		<jsp:param name="accessAction" value="<%=SelfAssessmentAction.ACCESS_ACTION %>"/>
...
<script type="text/javascript">
	...
	function exportProgress(){
		linkForm('listSelfAssessment_form', 'exportProgress', null, null);
	};
</script>