...
<table class="table hover" id="resultTable">
	<thead>
		<tr>
			...
			<th style="width: 20%" class="header <%=workflowDefineHolder.getCurrentSortClass("nameZH")%>">
			...
			<th style="width: 20%" class="header <%=workflowDefineHolder.getCurrentSortClass("nameEN")%>">
			...
			<th style="width: 10%" class="header <%=workflowDefineHolder.getCurrentSortClass("workflowModuleId")%>">
				<a onclick="submitForm('listworkflowDefine_form', null, null, 'workflowModuleId', '<%=workflowDefineHolder.getNextSortOrder("workflowModuleId")%>', 'tableWrapper');"><bean:message bundle="workflow" key="workflow.define.module" /></a>
			</th>
			<th style="width: 15%" class="header <%=workflowDefineHolder.getCurrentSortClass("approvalType")%>">
			...
			<th style="width: 15%" class="header <%=workflowDefineHolder.getCurrentSortClass("topPositionGrade")%>">
				<a onclick="submitForm('listworkflowDefine_form', null, null, 'topPositionGrade', '<%=workflowDefineHolder.getNextSortOrder("topPositionGrade")%>', 'tableWrapper');"><bean:message bundle="workflow" key="workflow.define.max.position.grade" /></a>
			</th>
			<th style="width: 10%" class="header <%=workflowDefineHolder.getCurrentSortClass("status")%>">
		...
		<tbody>
			<logic:iterate id="workflowDefineVO" name="workflowDefineHolder" property="source" indexId="number">
				<tr class='<%=number % 2 == 1 ? "odd" : "even"%>'>
					...
					<td><bean:write name="workflowDefineVO"  property="decodeWorkModuleId"/></td>
					<td><bean:write name="workflowDefineVO"  property="decodeApprovalType"/></td>
					<td><bean:write name="workflowDefineVO"  property="decodeMessageRemind"/></td>
					<td><bean:write name="workflowDefineVO"  property="decodeTopPositionGrade"/></td>
