	...
	<thead>
		<tr>
			...
			<th style="width: 10%" class="header <%=timesheetBatchHolder.getCurrentSortClass("batchId")%>">
				<a onclick="submitForm('searchTimesheetBatch_form', null, null, 'batchId', '<%=timesheetBatchHolder.getNextSortOrder("batchId")%>', 'tableWrapper');"><bean:message bundle="business" key="business.allowance.import.batch.id" /></a>
			</th>
			<th style="width: 10%" class="header <%=timesheetBatchHolder.getCurrentSortClass("monthly")%>">
				<a onclick="submitForm('searchTimesheetBatch_form', null, null, 'monthly', '<%=timesheetBatchHolder.getNextSortOrder("monthly")%>', 'tableWrapper');"><bean:message bundle="business" key="business.allowance.import.monthly" /></a>
			</th>
			...
			<th style="width: 15%" class="header  <%=timesheetBatchHolder.getCurrentSortClass("importExcelName")%>">
				<a onclick="submitForm('searchTimesheetBatch_form', null, null, 'importExcelName', '<%=timesheetBatchHolder.getNextSortOrder("importExcelName")%>', 'tableWrapper');"><bean:message bundle="business" key="business.allowance.import.file.name" /></a>
			</th>
			<th style="width: 10%" class="header <%=timesheetBatchHolder.getCurrentSortClass("createBy")%>">
				<a onclick="submitForm('searchTimesheetBatch_form', null, null, 'createBy', '<%=timesheetBatchHolder.getNextSortOrder("createBy")%>', 'tableWrapper');"><bean:message bundle="business" key="business.allowance.import.uploader" /></a>
			</th>
			
			<th style="width: 15%" class="header  <%=timesheetBatchHolder.getCurrentSortClass("createDate")%>">
				<a onclick="submitForm('searchTimesheetBatch_form', null, null, 'createDate', '<%=timesheetBatchHolder.getNextSortOrder("createDate")%>', 'createDate');"><bean:message bundle="business" key="business.allowance.import.upload.date" /></a>
			</th>
		...
		<tbody id="mytab">
			<logic:iterate id="timesheetBatchVO" name="timesheetBatchHolder" property="source" indexId="number">
				<tr class='<%=number % 2 == 1 ? "odd" : "even"%>'>
					...
					<td class="left">
						<a onclick="linkBatch('timesheetAllowanceHeaderAction.do?proc=list_object&batchId=<bean:write name="timesheetBatchVO" property="encodedId" />&batchStatus=<bean:write name="timesheetBatchVO" property="status" />');"><bean:write name="timesheetBatchVO" property="batchId" /></a>
					</td>
					<td class="left"><bean:write name="timesheetBatchVO" property="monthly" /></td>
					<td class="left">
						考勤表：<bean:write name="timesheetBatchVO" property="countHeaderId" />个；
						津贴总金额：<bean:write name="timesheetBatchVO" property="totalBases" />元；
<%-- 						休假：<bean:write name="timesheetBatchVO" property="totalLeaveHours" />小时； --%>
<%-- 						加班：<bean:write name="timesheetBatchVO" property="totalOTHours" />小时； --%>
						<!-- header记录小于3大于0 -->
