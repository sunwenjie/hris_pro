	...
	<div class="box searchForm toggableForm" id="TimesheetBatch-Information">
		<div class="head">
   	    ...
   	    <div id="searchDiv" class="inner" style="display: none;">
       		...
       		<html:form action="otImportBatchAction.do?proc=list_object" method="post" styleClass="searchTimesheetBatch_form">
	...
	<div class="box noHeader" id="search-results">
		<div class="inner">
			...
			<div class="top">
				...
				<kan:auth right="submit" action="<%=OTImportBatchAction.accessAction%>">
	            ...
	            <kan:auth right="import" action="<%=OTImportBatchAction.accessAction%>">
			...
			<div id="tableWrapper">
				...
				<jsp:include page="/contents/business/attendance/ot/table/listOTImportBatchTable.jsp" flush="true"/> 
			</div>
			<!-- tableWrapper -->
			<div class="bottom">
				<p>
...
<div id="popupWrapper">
	<jsp:include page="/popup/importExcel.jsp" >
		<jsp:param value="<%=OTHeaderAction.accessAction%>" name="accessAction"/>
		...
		$('#btnSubmit').click( function(){
			if($('#selectedIds').val() != null && $('#selectedIds').val() != ''){
				if(confirm('<bean:message bundle="public" key="popup.confirm.submit.selected.batch" />')){
					$('.searchTimesheetBatch_form').attr('action', 'otImportBatchAction.do?proc=submit_batch');
		...
		$('#btnBack').click( function(){
			if($('#selectedIds').val() != null && $('#selectedIds').val() != ''){
				if(confirm('<bean:message bundle="public" key="popup.confirm.return.selected.batch" />')){
					$('.searchTimesheetBatch_form').attr('action', 'otImportBatchAction.do?proc=back_batch');
					submitForm('searchTimesheetBatch_form', "submitObjects", null, null, null, 'tableWrapper');
					$('#selectedIds').val('');
				}
			}else{
