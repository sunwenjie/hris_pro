		...
		<tbody id="mytab">
			<logic:iterate id="timesheetBatchVO" name="timesheetBatchHolder" property="source" indexId="number">
				<tr class='<%=number % 2 == 1 ? "odd" : "even"%>'>
					...
					<td class="left">
						<a onclick="linkBatch('otImportHeaderAction.do?proc=list_object&batchId=<bean:write name="timesheetBatchVO" property="encodedId" />&batchStatus=<bean:write name="timesheetBatchVO" property="status" />');"><bean:write name="timesheetBatchVO" property="batchId" /></a>
					</td>
					<td class="left">
						加班数量：<bean:write name="timesheetBatchVO" property="countHeaderId" />；
						加班小时数：<bean:write name="timesheetBatchVO" property="totalOTHours" />；
						<!-- header记录小于3大于0 -->
