...
<div id="content">
	...
	<%= ManageRender.generateManage( request, "HRO_BIZ_EMPLOYEE_CONTRACT_OTHER", "employeeContractOtherForm", true ) %>
</div>
							
<script type="text/javascript">
	(function($) {	
		// JS of the List
		<%= ManageRender.generateManageJS( request, "HRO_BIZ_EMPLOYEE_CONTRACT_OTHER", "init();", null, null, "flag += validateBaseAndBaseFrom();" ) %>
		//雇员登陆限制隐藏按钮
		<%= EmployeeUserManageRender.generateDisplayBtnJS( request, "HRO_BIZ_EMPLOYEE_CONTRACT_OTHER")%>
	...
	function validateBaseAndBaseFrom(){
		$(".base_error2").remove();
		if ($(".base").val() == "" && $("#baseFrom").val() == "0") {
	        if ($(".base_error2").length <= 0) {
	            $(".base").after("<label class='error base_error2'>&#8226; 基数 或 基数来源 不能全为空</label>");
	        }
	        return 1;
	    }
	    return 0; 
	...
	function init(){
		...
		if('<bean:write name="role"/>' == 2){
			...
			$('#pageTitle').html('<bean:write name="employeeContractVO" property="decodeFlag" /> - 其他设置');
		}
