...
<div id="content">
	<div class="box searchForm toggableForm" id="employeeSalaryAdjustmentBatch-information">
		...
		<html:form action="employeeSalaryAdjustmentBatchAction.do?proc=list_object" styleClass="list_form">
			<input type="hidden" name="sortColumn" id="sortColumn" value="<bean:write name="employeeSalaryAdjustmentBatchHolder" property="sortColumn" />" />
			<input type="hidden" name="sortOrder" id="sortOrder" value="<bean:write name="employeeSalaryAdjustmentBatchHolder" property="sortOrder" />" />
			<input type="hidden" name="page" id="page" value="<bean:write name="employeeSalaryAdjustmentBatchHolder" property="page" />" />
			<input type="hidden" name="selectedIds" id="selectedIds" value="<bean:write name="employeeSalaryAdjustmentBatchHolder" property="selectedIds" />" />
	...
	<div class="box noHeader" id="search-results">
		<div class="inner">
			...
			<div class="top">
				...
				<kan:auth right="import" action="<%=EmployeeSalaryAdjustmentBatchAction.ACCESS_ACTION%>">
			...
			<!-- Include table jsp 包含tabel对应的jsp文件 -->  
				<jsp:include page="/contents/business/employee/salaryAdjustmentImport/table/listSalaryAdjustmentBatchTable.jsp" flush="true"/> 
			</div>
			<div class="bottom">
				<p>
			</div>
		...
		$('#btnSubmit').click( function(){
			if($('#selectedIds').val() != null && $('#selectedIds').val() != ''){
				if(confirm('<bean:message bundle="public" key="popup.confirm.submit.selected.batch" />')){
					$('.list_form').attr('action', 'employeeSalaryAdjustmentBatchAction.do?proc=submit_batch');
		...
		$('#btnDelete').click( function(){
			if($('#selectedIds').val() != null && $('#selectedIds').val() != ''){
				if(confirm('<bean:message bundle="public" key="popup.confirm.return.selected.batch" />')){
					$('.list_form').attr('action', 'employeeSalaryAdjustmentBatchAction.do?proc=rollback_batch');
