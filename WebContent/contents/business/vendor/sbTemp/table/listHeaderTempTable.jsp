	...
	<thead>
		<tr>
			...
			<th style="width: 6%" class="header <%=sbHeaderTempHolder.getCurrentSortClass("monthly")%>">
				<a onclick="submitForm('searchSBHeaderTemp_form', null, null, 'monthly', '<%=sbHeaderTempHolder.getNextSortOrder("monthly")%>', 'tableWrapper');">社保公积金申报月份</a>
			</th>
			<th style="width: 13%" class="header <%=sbHeaderTempHolder.getCurrentSortClass("employeeSBId")%>">
				<a onclick="submitForm('searchSBHeaderTemp_form', null, null, 'employeeSBId', '<%=sbHeaderTempHolder.getNextSortOrder("employeeSBId")%>', 'tableWrapper');">社保公积金名称</a>
			</th>
			...
			<th style="width: 11%" class="header <%=sbHeaderTempHolder.getCurrentSortClass("employeeNameZH")%>">
			...
			<th style="width: 11%" class="header <%=sbHeaderTempHolder.getCurrentSortClass("employeeNameEN")%>">
			...
			<th style="width: 5%" class="header <%=sbHeaderTempHolder.getCurrentSortClass("employStatus")%>">
				<a onclick="submitForm('searchSBHeaderTemp_form', null, null, 'employStatus', '<%=sbHeaderTempHolder.getNextSortOrder("employStatus")%>', 'tableWrapper');">员工状态</a>
			</th>
			<th style="width: 7%" class="header <%=sbHeaderTempHolder.getCurrentSortClass("sbStatus")%>">
				<a onclick="submitForm('searchSBHeaderTemp_form', null, null, 'sbStatus', '<%=sbHeaderTempHolder.getNextSortOrder("sbStatus")%>', 'tableWrapper');">社保公积金状态</a>
			</th>
			<th style="width: 8%" class="header <%=sbHeaderTempHolder.getCurrentSortClass("amountCompany")%>">
			...
			<th style="width: 8%" class="header <%=sbHeaderTempHolder.getCurrentSortClass("amountPersonal")%>">
		...
		<tbody id="mytab">
			<logic:iterate id="sbHeaderTempVO" name="sbHeaderTempHolder" property="source" indexId="number">
				<tr class='<%=number % 2 == 1 ? "odd" : "even"%>'>
					<td>
						<input type="checkbox" <logic:equal name="sbHeaderTempVO" property="tempStatus" value="2">class="hide"</logic:equal> id="kanList_chkSelectRecord_<bean:write name="sbHeaderTempVO" property="headerId"/>" name="chkSelectRow[]" value="<bean:write name="sbHeaderTempVO" property="encodedId"/>" />
					</td>
					<td class="left"><a onclick="link('sbDetailTempAction.do?proc=list_object&headerId=<bean:write name="sbHeaderTempVO" property="encodedId"/>');"><bean:write name="sbHeaderTempVO" property="headerId" /></a></td>
					<td class="left"><bean:write name="sbHeaderTempVO" property="monthly" /></td>
					<td class="left"><bean:write name="sbHeaderTempVO" property="employeeSBName" /></td>
					...
					<td class="left"><bean:write name="sbHeaderTempVO" property="decodeSbStatus" /></td>
