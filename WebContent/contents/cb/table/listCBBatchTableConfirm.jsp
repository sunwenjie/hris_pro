	...
	<thead>
		<tr>
			...
			<th style="width: 8%" class="header  <%=cbBatchHolder.getCurrentSortClass("amountSalesCost")%>">
				<a onclick="submitForm('list_form', null, null, 'amountPurchaseCost', '<%=cbBatchHolder.getNextSortOrder("amountPurchaseCost")%>', 'tableWrapper');"><bean:message bundle="cb" key="cb.batch.amount.sales.cost" /></a>
			</th>	 
		...
		<tbody>
			<logic:iterate id="cbBatchVO" name="cbBatchHolder" property="source" indexId="number">
				<tr class='<%=number % 2 == 1 ? "odd" : "even"%>'>
					<td><input type="checkbox" id="kanList_chkSelectRecord_<bean:write name="cbBatchVO" property="batchId"/>" name="chkSelectRow[]" value="<bean:write name="cbBatchVO" property="encodedId"/>" /></td>
					<td class="left"><a href="#" onclick="link('cbHeaderAction.do?proc=list_object&statusFlag=confirm&batchId=<bean:write name="cbBatchVO" property="encodedId"/>');"><bean:write name="cbBatchVO" property="batchId" /></a>&nbsp;&nbsp;&nbsp;<a href="#" onclick="link('cbHeaderAction.do?proc=list_object&statusFlag=confirm&batchId=<bean:write name="cbBatchVO" property="encodedId"/>');"></a></td>								
