...
<div class="top">
	<input type="button" id="btn_select" class="function" value="<bean:message bundle="public" key="button.invite" />" onclick="selectEmployee()"/>
	<input type="button" id="btn_cancel" name="btn_cancel" class="btn reset" onclick="popupInviteBox(false);" value="<bean:message bundle="public" key="button.exit" />" />
</div>
	...
	<thead>
		<tr>
			...
			<th style="width: 30%" class="header-nosort">
					<bean:message bundle="public" key="public.employee2.short.name" />
			</th>
			<th style="width: 30%" class="header <%=employeeHolder.getCurrentSortClass("nameZH")%>">
			...
			<th style="width: 30%" class="header <%=employeeHolder.getCurrentSortClass("nameEN")%>">
		...
		<tbody>
			<logic:iterate id="employeeVO" name="employeeHolder" property="source" indexId="certificateNumber">
				<tr class='<%=certificateNumber % 2 == 1 ? "odd" : "even"%>'>
					...
					<td class="left"><bean:write name="employeeVO" property="shortName" /></td>
